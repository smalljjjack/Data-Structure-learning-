...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    // Create an array to represent the walls
    int numOfWalls = vert * (horiz - 1) + horiz * (vert - 1);
    int[][] walls = new int[numOfWalls][3];
    for (i = 0; i < vert * (horiz - 1); i++) {			// vertical walls first element is 0
    	walls[i][0] = 0;
    	walls[i][1] = i % (horiz - 1);
    	walls[i][2] = i / (horiz - 1);
    }
    for (j = 0; j < horiz * (vert - 1); j++) {			// horizontal walls first element is 1
    	walls[j + vert * (horiz - 1)][0] = 1;
    	walls[j + vert * (horiz - 1)][1] = j % horiz;
    	walls[j + vert * (horiz - 1)][2] = j / horiz;
    }
   // System.out.println(arrayToString(walls));			// for debugging purpose only
    
    // Reorder the walls randomly
    for (int w = walls.length; w > 0; w--) {
    	int random = randInt(w);
    	int [] newLast = walls[random];
    	int [] last = walls[w - 1];
    	walls[w - 1] = newLast;
    	walls[random] = last;
    }
  System.out.println(arrayToString(walls));			// for debugging purpose only
    ...
    for (i = 0; i < numOfWalls; i++) {
    	...
    	int cell1, cell2;
    	if (walls[i][0] == 0) {
    		cell1 = walls[i][2] * horiz + walls[i][1];
    		cell2 = cell1 + 1;
    	} else {
    		cell1 = walls[i][2] * horiz + walls[i][1];
    		cell2 = cell1 + horiz;
    	...
    	// Remove the wall if no path connecting two cells
    	if (Maze.find(cell1) != Maze.find(cell2)) {
    		if (walls[i][0] == 0) {
    			vWalls[walls[i][1]][walls[i][2]] = false;
    		} else {
    			hWalls[walls[i][1]][walls[i][2]] = false;
    		}
    		Maze.union(Maze.find(cell1), Maze.find(cell2));
    	}
    }
  ...
  private String arrayToString(int[][] walls) {
	String str = "";
